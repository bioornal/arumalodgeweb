...
## Cambios recientes
- **2026-07-18:**
  - **Auto-degradado de efectos** (`FxWatchdog` en el layout): mide FPS reales al cargar (600ms de warmup + 1.8s de muestreo); si el promedio queda bajo 24fps — o si la muestra no terminó a los 4s (deadman: rAF muerto por freeze duro del compositor) — graba `aruma-fx-off` en localStorage (TTL 24h) y recarga — el script inline del layout lee el flag antes de hidratar y arranca con `data-fx=""` (mismo mecanismo de `?sinfx=1`). La URL siempre le gana al flag. Contexto: el freeze en máquinas sin aceleración de GPU persistía de forma intermitente incluso después de redimensionar las fotos del bucket (los originales de 8–12MP se bajaron a 1920px máx, ~62% menos RAM de decodificación).
